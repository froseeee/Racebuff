...
def is_pid_exist() -> bool:
    ...
    try:
        ...
        # Verify if last PID is running and belongs to RaceBuff
...
def single_instance_check(is_single_instance: bool):
    ...
    # Skip if restarted
    if os.getenv("RACEBUFF_RESTART") == "TRUE":
        os.environ.pop("RACEBUFF_RESTART", None)
        save_pid_file()
        return
...
def get_version():
    """Get version info"""
    logger.info("RaceBuff: %s", VERSION)
...
def init_gui() -> QApplication:
    """Initialize Qt Gui"""
    # Set global locale (UI language is applied via locale_i18n.tr())
    ...
    if PLATFORM != "Windows":
        root.setDesktopFileName("RaceBuff-overlay")
    # Set default font
    font = root.font()
