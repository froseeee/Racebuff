...
class PresetTransfer(BaseEditor):
    ...
    def transfer(self):
        ...
        if not self.dest_selector.currentText():
            msg_text = tr("No destination preset selected or found.")
        ...
        if not setting_selection:
            msg_text = tr("No preset setting selected.<br><br>Select at least one setting and try again.")
            QMessageBox.warning(self, tr("Error"), msg_text)
            return
        options_selection = tuple(self.get_setting_selection(self.listbox_options))
        if not options_selection:
            msg_text = tr("No option type selected.<br><br>Select at least one option type and try again.")
            QMessageBox.warning(self, tr("Error"), msg_text)
            return
        msg_text = (
        ...
        QMessageBox.information(self, tr("Transfer Completed"), msg_text)
