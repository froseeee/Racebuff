...
class ModuleList(QWidget):
    ...
    def create_list(self):
        ...
        for _name in self.module_control.names:
            ...
            item.setText(tr(format_module_name(_name)))
    ...
    def refresh_label(self):
        """Refresh label text"""
        n = self.module_control.number_active
        total = self.module_control.number_total
        self.label_loaded.setText(
            tr("Enabled: <b>{n}/{total}</b>", n=n, total=total)
        )
    ...
    def confirm_batch_toggle(self, confirm_type: str) -> bool:
        ...
        if not cfg.application["show_confirmation_for_batch_toggle"]:
            return True
        tid = self.module_control.type_id
        key = f"{confirm_type} all {tid}s?"
        msg_text = tr(key)
...
class ModuleControlItem(QWidget):
    ...
    def update_button_text(self):
        """Update button text"""
        self.button_toggle.setText(tr("ON") if self.is_enabled() else tr("OFF"))
