...
class FontConfig(BaseDialog):
    ...
    def __init__(self, parent, user_setting: dict, reload_func: Callable):
        ...
        layout_option.addWidget(QLabel(tr("Font Name")), 0, 0)
        layout_option.addWidget(self.edit_fontname, 0, 1)
        layout_option.addWidget(QLabel(tr("Font Size Addend")), 1, 0)
        layout_option.addWidget(self.edit_fontsize, 1, 1)
        layout_option.addWidget(QLabel(tr("Font Weight")), 2, 0)
...
class UserConfig(BaseDialog):
    ...
    def reset_setting(self):
        """Reset setting"""
        key_display = format_option_name(self.key_name)
        msg_text = tr(
    ...
    def value_error_message(self, value_type: str, option_name: str):
        """Value error message"""
        option_display = format_option_name(option_name)
        msg_text = tr(
            "Invalid {value_type} for <b>{option_name}</b> option.<br><br>Changes are not saved.",
            value_type=value_type, option_name=option_display,
        )
    ...
    def __add_option_label(self, idx, key, layout):
        """Option label"""
        label_text = tr(format_option_name(key))
